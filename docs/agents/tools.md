...
| Workflow | `workflow` | `grok_build/workflow/` + `xai-workflow` + shell `session/workflow/` | Rhai orchestration over subagents; always background |
...
- `workflow` launches a **Rhai** orchestration script through the native workflow runtime: the pure engine lives in `crates/codegen/xai-workflow/` (interpreter, journal, meta/validate) and the session binding in `xai-grok-shell/src/session/workflow/` (`manager`, `host_service`, `tracker`, `store`, `registry`, `notify`, `schema_contract`). The script must begin with `let meta = #{ name, description, when_to_use?, phases? }` as a literal; the body drives `agent()`, `parallel()`, `phase()`, `log()`, `complete()`, `pause()`, `await_user()`, `budget()`, `render_template()`, scratch-file helpers, `git_diff_since()`, `fingerprint()`, and `json_encode()`.
- The tool takes exactly one source — `name` (registry), inline `script`, or `script_path` — plus optional `args`, `agent_budget` (default 128, max 1024 agent calls), `resume_from_run_id`, and `validate_only`. Launch is decoupled: the tool sends a `WorkflowLaunchRequest` over an mpsc handle; the session resolves the source and `WorkflowManager::launch()` runs the interpreter on `spawn_blocking`, capped at 4 active runs per session.
- Every `agent()` call is serviced by `host_service.rs`: it builds a `SubagentRequest` (model/reasoning_effort/agent_type/capability_mode/worktree isolation from the opts map, `owner = SubagentOwner::workflow(run_id)`) and sends `SubagentEvent::Spawn` down the same coordinator path as `task`/`agent_swarm` — so model and effort overrides, including `antigravity:` slugs, resolve identically. `output_schema` compiles a JSON-Schema contract with fenced-JSON extraction and one corrective retry.
- Determinism + replay: an append-only JSONL journal under `<session>/workflows/<run_id>/` records every host call by canonical request hash; resume replays journaled calls without re-running and fails loudly on divergence. `timestamp()`/`sleep()`/`exit()` are blocked in-script. Resume requires the byte-identical immutable script + args; budget-limited runs resume only with a higher `agent_budget`; process restarts demote active runs to `Interrupted` (terminal).
- Lifecycle + UI: runs are always background; state changes broadcast as `WorkflowUpdated` ext-notifications that feed the pager's inline workflow scrollback block and the `/workflows` overlay (drill-down, pause/resume/stop/save). Terminal states inject a `WorkflowCompletionTurn` back into the model's context — no polling. `/workflow pause|resume|stop|save <name>` manages runs by display name.
- Registry: builtin (`deep-research`, `include_str!`'d) > project (`<repo>/.opengrok/workflows/*.rhai`, folder-trust + `O_NOFOLLOW` gated) > user (`~/.opengrok/workflows/*.rhai`); registered workflows also surface as their own slash commands. Depth rules unchanged: workflows launch at depth 0 only, and child toolsets strip `task`/`agent_swarm`/`workflow` at depth 1.
