...
/// Test-only key override. Available under `cfg(test)` (unit tests) or the
/// `test-support` feature (integration tests in downstream crates). Never release.
#[cfg(any(all(test, debug_assertions), feature = "test-support"))]
pub mod test_seam {
    ...
    /// Process keys: `None` clear (restore compiled-in), `Some(&[])` dark,
    /// `Some(non-empty)` override.
    pub fn set_embedded_keys_opt(keys: Option<&[(&str, &[u8])]>) {
        *GLOBAL_OVERRIDE.write().unwrap_or_else(|e| e.into_inner()) = to_owned_keys(keys);
    }

    /// Install throwaway trusted keys for integration tests (slice form).
    /// Empty slice forces dark. To restore compiled-in keys, call
    /// [`set_embedded_keys_opt`] with `None`.
    pub fn set_embedded_keys(keys: &[(&str, &[u8])]) {
        set_embedded_keys_opt(Some(keys));
    }
...
fn with_embedded_keys<R>(f: impl FnOnce(&[(&str, &[u8])]) -> R) -> R {
    ...
    #[cfg(not(any(all(test, debug_assertions), feature = "test-support")))]
    {
