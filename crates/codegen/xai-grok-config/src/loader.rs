...
/// User config filename (`$OPENGROK_HOME/config.toml`), shared by the loaders here.
...
pub enum HookProvenance {
    /// `/etc/opengrok/managed_config.toml`.
    SystemManaged,
    /// `$OPENGROK_HOME/managed_config.toml` (server-synced).
    Managed,
