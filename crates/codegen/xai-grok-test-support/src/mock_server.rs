...
pub struct MockInferenceServer {
    ...
    /// Scripted responses consumed only by tool-bearing agent turns (see
    /// [`enqueue_agent_turn_response`]). Distinct from the per-path FIFO.
    ///
    /// [`enqueue_agent_turn_response`]: Self::enqueue_agent_turn_response
    scripted_agent_turns: Arc<std::sync::Mutex<VecDeque<ScriptedResponse>>>,
...
impl MockInferenceServer {
    ...
    async fn start_inner(
        ...
        let scripted_agent_turns: Arc<std::sync::Mutex<VecDeque<ScriptedResponse>>> =
    ...
    /// this queue; an empty queue falls through to the normal dispatch.
    ...
    /// A tool-bearing agent turn carries 2+ tools; auxiliary requests
    /// (title/classifier) carry fewer, so they never consume an agent-turn
    /// scripted response.
                ...
                post(move |headers: HeaderMap, Json(body): Json<Value>| {
                    ...
                    async move {
                        ...
                        if let Some(s) = Self::pop_scripted_agent_turn(&scripted_agent_turns, &body)
                        {
                            ...
                            return s.into_response_paced(chunk_delay, None).await;
                        }
