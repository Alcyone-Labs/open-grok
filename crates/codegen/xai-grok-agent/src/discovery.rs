...
//!
//! Searches `.opengrok/agents/` and `.claude/agents/` from cwd to repo root,
//! then `~/.opengrok/agents/`, then `~/.claude/agents/`. Name-based dedup keeps
//! highest priority.
...
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".opengrok/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.opengrok/agents` / `.claude/agents`), walked
...
/// Existing project agent dirs (`.opengrok/agents` / `.claude/agents`) under each
...
fn merge_subagent_candidates(discovered: Vec<AgentDefinition>) -> Vec<SubagentEntry> {
    ...
    //
    // A user-level ~/.opengrok/agents/explore.md does NOT shadow built-in explore
...
/// 1. `.opengrok/agents/` walking from `cwd` up to repo root
/// 2. `~/.opengrok/agents/` (user-level)
/// 3. `~/.claude/agents/` (compat user-level)
/// 4. `~/.opengrok/bundled/agents/` (bundled, lowest priority)
///
...
/// compat agents, then bundled. `.opengrok` dirs resolve from `grok_home`
/// (OPENGROK_HOME-aware) plus the legacy literal `~/.opengrok` when OPENGROK_HOME points
/// elsewhere; `.claude` resolves from `home`.
pub(crate) fn user_agent_dirs(
    home: Option<&Path>,
    grok_home: Option<&Path>,
) -> Vec<(std::path::PathBuf, AgentScope)> {
    // Legacy literal ~/.opengrok, included only when it differs from grok_home
    // (i.e. OPENGROK_HOME points elsewhere) so agents left in the old location are
...
///
/// Project-level `.opengrok/agents/` has highest priority, then falls back
...
/// Load project agent definitions from every `.opengrok/agents` / `.claude/agents`
...
mod tests {
    ...
    fn user_agent_dirs_includes_legacy_grok_when_grok_home_differs() {
        ...
        assert!(paths.contains(&home.join(".opengrok").join("agents")));
        assert!(paths.contains(&home.join(".claude").join("agents")));
        assert!(paths.contains(&grok.join("bundled").join("agents")));
        assert!(paths.contains(&home.join(".opengrok").join("bundled").join("agents")));
    }
    ...
    fn user_agent_dirs_dedups_legacy_when_grok_home_is_dot_grok() {
        ...
        assert_eq!(
            count, 1,
            "no duplicate ~/.opengrok/agents when grok_home == ~/.opengrok"
        );
    }
    ...
    fn test_discover_finds_md_files() {
        ...
        let agents_dir = tmp.path().join(".opengrok").join("agents");
    ...
    fn test_discover_dedup_by_name() {
        ...
        let agents_dir_1 = tmp.path().join(".opengrok").join("agents");
        let agents_dir_2 = inner_dir.join(".opengrok").join("agents");
    ...
    fn test_discover_includes_bundled_agents_at_lowest_priority() {
        ...
        let bundled_dir = home.join(".opengrok").join("bundled").join("agents");
        ...
        let defs = discover_with_home(&cwd, Some(&home), Some(&home.join(".opengrok")));
        assert_eq!(defs.len(), 1);
    ...
    fn test_by_name_in_cwd_user_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let user_dir = home.join(".opengrok").join("agents");
        let bundled_dir = home.join(".opengrok").join("bundled").join("agents");
        fs::create_dir_all(&cwd).unwrap();
        ...
        let def =
            by_name_in_cwd_with_home("reviewer", &cwd, Some(&home), Some(&home.join(".opengrok")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_builtin_beats_bundled_for_builtin_names() {
        ...
        let def =
            by_name_in_cwd_with_home("explore", &cwd, Some(&home), Some(&home.join(".opengrok")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_project_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let project_dir = cwd.join(".opengrok").join("agents");
        let bundled_dir = home.join(".opengrok").join("bundled").join("agents");
    ...
    fn test_by_name_in_cwd_falls_back_to_builtin() {
        let tmp = tempfile::tempdir().unwrap();
        // No .opengrok/agents/ directory — should fall back to built-in
    ...
    fn test_merge_user_level_builtin_name_is_skipped() {
        // A user-level (~/.opengrok/agents/) agent named "explore" should NOT shadow
