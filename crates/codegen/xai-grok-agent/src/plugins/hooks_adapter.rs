...
mod tests {
    ...
    /// The adapter recomputes the command from the unexpanded `command_raw`
    /// (substituting plugin tokens before the generic env pass), so the
    /// parser's own expansion pass — whatever it resolved — never stacks with
    /// the adapter's.
    #[test]
