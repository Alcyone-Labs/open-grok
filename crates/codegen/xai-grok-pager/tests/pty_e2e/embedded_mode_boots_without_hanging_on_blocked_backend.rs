...
async fn embedded_mode_boots_without_hanging_on_blocked_backend() {
    ...
    // Open Grok: state under OPENGROK_HOME / ~/.opengrok — never ~/.grok.
    let opengrok_home = home.path().join(".opengrok");
    std::fs::create_dir_all(&opengrok_home).unwrap();
    let env = [
        ("HOME", home.path().to_str().unwrap()),
        ("OPENGROK_HOME", opengrok_home.to_str().unwrap()),
