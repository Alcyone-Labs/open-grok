...
fn assert_set_bool_action(outcome: SettingsKeyOutcome, key: &str, expected: bool) {
    ...
    match (key, action) {
        ...
        (
            ...
            enabled, expected,
            "SetSwarmMode value differs from expected"
        ),
        ...
        ("toolset.perplexity_web_search.enabled", Action::SetPerplexityWebSearch(b)) => assert_eq!(
            b, expected,
            "SetPerplexityWebSearch value differs from expected"
        ),
...
#[test]
fn space_on_swarm_mode_dispatches_typed_setter() {
    let mut s = make_state();
    navigate_to(&mut s, "swarm_mode");
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Char(' ')));
    assert_set_bool_action(outcome, "swarm_mode", true);
}

#[test]
fn space_on_perplexity_web_search_dispatches_typed_setter() {
    let mut s = make_state();
    navigate_to(&mut s, "toolset.perplexity_web_search.enabled");
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Char(' ')));
    assert_set_bool_action(outcome, "toolset.perplexity_web_search.enabled", true);
}
...
#[test]
fn enter_on_code_mode_opens_picker_and_commits_typed_setter() {
    let mut s = make_state();
...
#[test]
fn mouse_click_on_swarm_mode_indicator_toggles_in_one_click() {
    let mut s = make_state();
    synth_rects(&mut s);
    let row_y = row_idx_for(&s, "swarm_mode") as u16;
    let outcome = handle_settings_mouse(
        &mut s,
        MouseEventKind::Down(crossterm::event::MouseButton::Left),
        72,
...
#[test]
fn mouse_click_on_perplexity_web_search_indicator_toggles_in_one_click() {
    let mut s = make_state();
    synth_rects(&mut s);
    let row_y = row_idx_for(&s, "toolset.perplexity_web_search.enabled") as u16;
    let outcome = handle_settings_mouse(
        &mut s,
        MouseEventKind::Down(crossterm::event::MouseButton::Left),
        72,
...
fn settings_value_payload_matches_kind() {
    ...
    for meta in reg.all() {
        ...
        match outcome {
            ...
            | SettingsKeyOutcome::Action(Action::SetSwarmMode { .. })
...
/// Up/Down/j/k navigation in the `screen_mode` picker must not dispatch a
/// preview action because the setting is restart-required.
#[test]
fn screen_mode_picker_nav_does_not_dispatch_preview() {
    for nav_key in &[
        ...
        assert!(
            matches!(outcome, SettingsKeyOutcome::Changed),
            "navigation key {nav_key:?} in screen_mode picker must not dispatch a preview \
             action; got {outcome:?}",
        );
        assert!(matches!(s.mode(), SettingsModalMode::PickingEnum { .. }));
    }
...
/// Enter commits the focused registry canonical through SetScreenMode.
#[test]
...
/// The choice catalog is the canonical contract with the screen-mode parser.
#[test]
fn screen_mode_choices_use_canonical_strings() {
    ...
    assert_eq!(canonicals, vec!["fullscreen", "minimal"]);
    match &meta.kind {
        SettingKind::Enum {
            supports_preview, ..
        } => assert!(
            !*supports_preview,
            "screen_mode is restart-required and must not preview live"
        ),
        _ => unreachable!(),
    }
...
/// Value-column click opens the screen-mode picker in one click.
#[test]
