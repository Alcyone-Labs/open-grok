...
fn shell_aliases_expand_to_exact_argv_and_bypass_is_explicit() {
    ...
    // Use the fork's real binary name (`open-grok`) as the alias stand-in.
    // An interactive `bash -ic` sources the user's rc, which on macOS reorders
    // PATH via `path_helper` and can prepend a real upstream `grok` ahead of
    // this tempdir — shadowing the stub. `open-grok` lives under `~/.opengrok`
    // (not the polluted early PATH), so the stub resolves reliably and the
    // scenario matches what the fork actually ships.
    let open_grok = temp.path().join("open-grok");
    std::fs::write(
        &open_grok,
        format!(
            "#!/bin/sh\nprintf '%s\\n' \"$@\" > '{}'\n",
    ...
    .unwrap();
    use std::os::unix::fs::PermissionsExt as _;
    std::fs::set_permissions(&open_grok, std::fs::Permissions::from_mode(0o755)).unwrap();

    if let Some(bash) = find_on_path("bash") {
        let rc = temp.path().join("bashrc");
        std::fs::write(&rc, "alias ssh='open-grok wrap ssh'\n").unwrap();
        let command = format!(
    ...
    let mut shell = std::process::Command::new(bash);
    shell
        .args(["-ic", "alias ssh='open-grok wrap ssh'; command ssh host"])
        .env("CAPTURE", &capture)
        .env(
            "PATH",
            format!(
