...
use crate::settings::{
    CodingDataSharingLock, EnumChoice, PagerLocalSnapshot, SecretInput, SettingCategory,
...
fn kimi_secret_editor_is_always_empty_and_empty_enter_preserves() {
    for status in [
        ...
        assert!(matches!(
            &state.state.mode,
            SettingsMode::EditingSecret { buffer, cursor_byte: 0, .. }
                if buffer.is_empty()
        ));
...
fn kimi_secret_paste_is_masked_redacted_and_commits_typed_action() {
    ...
    assert!(matches!(
        handle_settings_secret_paste(&mut state, SecretInput::new(format!("  {secret}\n")),),
...
fn perplexity_secret_paste_is_masked_and_commits_typed_action() {
    ...
    assert!(matches!(
        handle_settings_secret_paste(&mut state, SecretInput::new(secret.to_owned())),
        SettingsKeyOutcome::Changed
    ));
    let outcome = handle_settings_key(
        &mut state,
...
fn kimi_secret_rejects_internal_whitespace_without_retaining_payload() {
    ...
    assert!(matches!(
        handle_settings_secret_paste(&mut state, SecretInput::new("secret with-space".to_owned()),),
        SettingsKeyOutcome::Changed
    ));
    assert!(matches!(
        &state.state.mode,
...
fn picker_highlights_current_choice() {
    ...
    assert_eq!(
        bg_at(4),
        Some(super::render::settings_list_row_bg(&theme, true, false,)),
        "focused row must use the canonical selected background"
    );
    assert_eq!(
        bg_at(3),
        Some(super::render::settings_list_row_bg(&theme, false, false,)),
        "unfocused row must use the canonical base background"
    );
...
fn locked_coding_data_sharing_row_does_not_open_picker() {
    for lock in [
        ...
        CodingDataSharingLock::PolicyLock,
    ] {
...
/// value: ZDR replaces opt-in/out with "ZDR"; policy-managed keeps the
/// value with a " · Policy Managed" suffix. Unlocked rows keep the plain
/// value + chevron.
#[test]
fn locked_coding_data_sharing_row_renders_locked_value_without_chevron() {
    ...
    let mut s = make_locked_state(CodingDataSharingLock::PolicyLock);
    s.selected = idx;
    let mut buf = Buffer::empty(area);
    render_rows(&mut buf, area, &mut s, &theme);
    let rect = s.row_rects[idx];
    let line = buf_row_text(&buf, rect.y, area.x, area.width);
    assert!(
        line.contains("Opt out \u{00B7} Policy Managed"),
        "policy-managed lock must append ` · Policy Managed`: {line:?}"
    );
    assert!(
        !line.contains(chevron),
        "locked row must not render the `{chevron}` enter affordance: {line:?}"
...
fn locked_coding_data_sharing_expanded_description_replaces_with_reason() {
    ...
    assert!(
        text.contains("Locked by your organization's policy."),
    ...
    assert!(
        !text.contains("Locked by your organization's policy."),
        "unlocked expansion must not mention the policy lock: {text:?}"
    );
}
