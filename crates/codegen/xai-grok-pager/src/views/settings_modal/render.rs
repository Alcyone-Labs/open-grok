...
pub fn render_settings_modal(
    ...
    let title: &str = if let Some(o) = overlay {
        ...
        match &state.state.mode {
            ...
            | SettingsMode::EditingSecret { key, .. } => {
                if let Some(meta) = state.registry.find(key) {
                    breadcrumb_owned =
                        format!("{MODAL_TITLE} {} {}", crate::glyphs::chevron(), meta.label);
                    &breadcrumb_owned
    ...
    let (inner_area, docs_footer_area) = match state.state.mode_kind() {
        SettingsModeKind::EditingString
        | SettingsModeKind::EditingInt
        | SettingsModeKind::EditingSecret => (content_area, None),
...
pub(super) fn render_editing_value(
    ...
    let (setting_key, buffer_owned, cursor_byte, validation_error_owned) = match &state.state.mode {
        ...
        _ => unreachable!("editor renderer requires String, Secret, or Int state"),
    };
    let mut display_editor = crate::input::line_editor::LineEditor::default();
    display_editor.set_text(buffer_owned);
    let _ = display_editor.set_cursor_byte(cursor_byte);
    let buffer = display_editor.text();
    ...
    if buffer.is_empty() {
        ...
        let viewport = display_editor.viewport(buffer_room);
...
/// Resolve a dynamic enum's persisted canonical to the friendly label shown in
/// the settings row while keeping model IDs stable on disk.
fn display_for_dynamic_canonical(
    kind: &SettingKind,
    canonical: &str,
    snapshot: &crate::settings::PagerLocalSnapshot,
) -> String {
...
pub(super) const ROW_ADMIN_MANAGED_SUFFIX: &str = " \u{00B7} Policy Managed";
...
pub(super) fn value_display(
    ...
    pager: &crate::settings::PagerLocalSnapshot,
) -> String {
    ...
    let mut display = match value {
        ...
        SettingValue::String(s) => display_for_dynamic_canonical(&meta.kind, s, pager),
    ...
    if lock == Some(CodingDataSharingLock::PolicyLock) {
...
pub(super) fn render_setting_row(
    ...
    // Value text for paint. DynamicEnum callers may already rewrite
    // `value` to a display string; lock overlays apply on top.
    let value_text = if lock == Some(CodingDataSharingLock::Zdr) {
        ROW_ZDR_VALUE.to_string()
    } else {
        let mut display = match value {
            SettingValue::Bool(b) => if *b { "on" } else { "off" }.to_string(),
    ...
    let value_style = if lock.is_some()
        || matches!(
            value,
...
pub(super) fn build_shortcuts(state: &SettingsModalState) -> Vec<Shortcut<'static>> {
    match &state.state.mode {
        SettingsMode::Browse => {
            // A locked row (ZDR / policy-managed) accepts neither the edit keys
        ...
        SettingsMode::EditingSecret { .. } => vec![
            Shortcut {
