...
/// User-level vs project-level config files (`~/.opengrok` vs `{cwd}/.opengrok`).
...
/// Union bundled personas with local `~/.opengrok/personas` and `{cwd}/.opengrok/personas`.
///
...
pub fn merge_persona_lists(bundle: &BundleState, cwd: &Path) -> Vec<PersonaDetail> {
    ...
    let dirs = [
        (
            ...
            cwd.join(".opengrok").join("personas"),
        ),
...
fn personas_dir_for_scope(scope: ConfigFileScope, cwd: &Path) -> PathBuf {
    match scope {
        ...
        ConfigFileScope::Project => cwd.join(".opengrok").join("personas"),
    }
}
...
/// True when `path` is a deletable local persona file (user or project `.opengrok/personas`).
pub fn persona_path_is_deletable(path: &Path) -> bool {
    config_path_is_user_or_project(path, "personas")
}
/// Shared guard: canonical path under `~/.opengrok/{subdir}` or `{cwd}/.opengrok/{subdir}`, not bundled.
fn config_path_is_user_or_project(path: &Path, subdir: &str) -> bool {
    ...
    let project_suffix = std::path::Path::new(".opengrok").join(subdir);
...
mod tests {
    ...
    fn merge_persona_lists_appends_local_only() {
        ...
        let personas_dir = dir.path().join(".opengrok").join("personas");
    ...
    fn persona_is_deletable_local_vs_bundled() {
        let dir = tempfile::tempdir().expect("tempdir");
        let local = dir.path().join(".opengrok").join("personas").join("p.toml");
        ...
        let bundled_detail = PersonaDetail {
            ...
            has_inputs: false,
            has_outputs: false,
            source_path: Some("/home/user/.opengrok/bundled/personas/b.toml".into()),
            scope_label: None,
        };
