...
pub enum Action {
    ...
    OpenTranscriptPager,
    /// Minimal mode (`open-grok --minimal`): re-print the most-recently committed
...
///
/// Both variants write to `~/.opengrok/config.toml` and route ACP
...
pub enum Effect {
    ...
    /// Persist a typed setting to `~/.opengrok/config.toml`. On failure,
