...
impl AgentView {
    ...
    ///
    /// The draw path inlines the overlay/sub-mode dispatch (block viewer,
    /// permission queue, plan approval, question, cancel-turn) and delegates
    /// here for the normal-pane case, so this stays the single source of truth
    /// for that block.
    ///
    /// `esc_owned_before_agent`: app-level Esc ownership snapshot
    /// (`AppView::esc_owned_before_agent`), threaded through so the bar never
    /// advertises `Esc cancel` while an app-level owner would consume it.
    pub(crate) fn normal_pane_hints(
        &self,
    ...
    pub fn draw(
        ...
        let turn_status_height = if turn_status::should_show(
            &self.session.state,
            drain_blocked,
            self.mcp_init_progress
                .as_ref()
                .filter(|_| !self.session.loading_replay),
            watchers,
            parked,
        ) {
            1
            ...
            if let Some(ref pav) = self.plan_approval_view {
                ...
                let turn_output = turn_status::render_turn_status(
                    ...
                    turn_status::TurnStatusArgs {
                        ...
                        // Fork: suppress MCP seed chrome while replaying history.
                        mcp_init_progress: self
                            .mcp_init_progress
                            .as_ref()
                            .filter(|_| !self.session.loading_replay),
        ...
        let agent_name = self.session_agent_name.as_deref().filter(|s| !s.is_empty());
