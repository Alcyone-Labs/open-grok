...
fn print_exit_resume_hint(info: &ExitInfo, max_width: usize, w: &mut impl Write) {
    ...
    if info.minimal {
        let _ = writeln!(w, "  open-grok --minimal --resume {}", info.session_id);
    } else {
        let _ = writeln!(w, "  open-grok --resume {}", info.session_id);
    }
}
...
mod tests {
    ...
    fn terminal_title_strips_control_characters() {
        assert_eq!(
            ...
            "evil]52;c;payloadtitle - open-grok"
        );
        assert_eq!(terminal_title_string("\x07\x1b\x00"), "open-grok");
        assert_eq!(terminal_title_string(""), "open-grok");
        assert_eq!(terminal_title_string("My chat"), "My chat - open-grok");
    }
    #[test]
    ...
    #[test]
    fn cli_command_name_is_open_grok() {
        use clap::CommandFactory;
        assert_eq!(PagerArgs::command().get_name(), "open-grok");
    }
    #[test]
    ...
    fn print_exit_resume_hint_includes_minimal_flag() {
        ...
        assert_eq!(
            String::from_utf8(buf).unwrap(),
            "\nResume this session with:\n  open-grok --minimal --resume sess-abc\n"
        );
    }
    #[test]
    ...
    fn print_exit_resume_hint_truncates_summary_to_width() {
        ...
        assert!(out.contains(&format!("\n> {}…\n", "p".repeat(17))));
        assert!(out.contains(&format!("\n  {}…\n", "r".repeat(17))));
        assert!(out.contains("  open-grok --resume sess-abc\n"));
    }
    #[test]
