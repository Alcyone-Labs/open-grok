...
/// Open Grok home for the replay path. In production this is just `grok_home()`
/// (`$OPENGROK_HOME` / `~/.opengrok`); the whole test override below is
/// `#[cfg(test)]`, so no thread-local or dead always-false branch ships in
/// release.
#[cfg(not(test))]
...
/// Override Open Grok home for disk-replay unit tests (thread-local).
#[cfg(test)]
