...
pub struct AppView {
    ...
    /// Explicit auxiliary-model pins loaded from `[models]` and mirrored
    ...
    /// Current appearance config (hot-reloadable from ~/.opengrok/pager.toml).
    ...
    /// Dashboard entry points (`/dashboard`, `Ctrl+\`, `open-grok dashboard`, the
    ...
    /// `~/.opengrok/config.toml`. `None` when the file/section is absent
...
impl AppView {
    ...
    pub fn coding_data_sharing_lock(&self) -> Option<crate::settings::CodingDataSharingLock> {
        if self.is_zdr {
            ...
            Some(crate::settings::CodingDataSharingLock::PolicyLock)
        } else {
            None
        }
    }
    ...
    /// Mirror the provider-neutral `/usage` command gate onto every slash
    /// surface that can run `/usage` (agents, welcome, dashboard dispatch /
    /// peek-reply). Pushes [`Self::usage_command_visible`] so a Codex-only
    /// session keeps `/usage` even when the xAI billing surface is hidden.
...
impl AppView {
    ...
    pub(crate) fn handle_owned_input_with_paste_provenance(
        ...
        self.handle_owned_input_at_with_paste_provenance(ev, Instant::now(), paste_provenance)
    }

    pub(crate) fn handle_owned_input_at_with_paste_provenance(
        &mut self,
        ev: &mut Event,
        arrived_at: Instant,
        paste_provenance: PasteProvenance,
        ...
        self.handle_input_at_with_paste_provenance(ev, arrived_at, paste_provenance)
    }
                ...
                match *active_view {
                    ...
                    ActiveView::AgentDashboard => {
                        if let Some(dashboard) = self.dashboard.as_mut() {
                            ...
                            let cursor = if tutorial_open || dashboard_settings_open {
                                None
    ...
    fn is_scroll_blocking_modal_open(&self) -> bool {
        ...
        matches!(self.active_view, ActiveView::Agent(id) if self.agents.get(&id).is_some_and(|a| a.extensions_modal.is_some() || a.active_modal.is_some()))
            ...
            || matches!(
                self.active_view,
                ActiveView::AgentDashboard
                    if self.dashboard.as_ref().is_some_and(|d| {
...
pub(crate) mod tests {
    ...
    fn owned_secret_paste_moves_text_out_of_terminal_event() {
        ...
        settings.enter_secret_editor_for_test("kimi_api_key");
        ...
        assert!(state.secret_editor_matches_for_test("sk-owned-secret"));
    }
    #[test]
    fn dashboard_owned_secret_paste_moves_text_out_of_terminal_event() {
        ...
        assert!(state.secret_editor_matches_for_test("sk-dashboard-secret"));
    ...
    fn overlay_provider_secret_editor_owns_cycle_and_exit_shortcuts() {
        ...
        assert!(state.secret_editor_matches_for_test("s"));
    }
