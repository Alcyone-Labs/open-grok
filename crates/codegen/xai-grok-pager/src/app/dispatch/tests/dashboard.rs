...
/// `open-grok dashboard` before login: the startup hook consumes the
...
fn auth_complete_opens_deferred_dashboard() {
    ...
    assert!(
        ...
        "deferred `open-grok dashboard` must open the dashboard after login",
    );
    assert!(
...
fn dashboard_image_dispatch_cancel_rewind_resends_attachment() {
        ...
        .unwrap();
    {
        ...
        assert!(matches!(
            maybe_drain_queue(agent, new_id).effects.as_slice(),
...
/// `open-grok dashboard`, and the startup hook.
#[serial_test::serial(GROK_AGENT_DASHBOARD)]
#[test]
...
fn dashboard_attach_subagent_switches_to_parent_with_subagent_focused() {
    ...
    assert!(
        !parent_view.subagent_views[&child_sid]
            .normal_pane_hints(&app.registry, false)
            .iter()
