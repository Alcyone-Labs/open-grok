...
use super::settings::setters::{
    ...
    clear_memory_model, clear_perplexity_api_key, clear_recap_model, preview_auto_dark_theme,
    ...
    set_local_feature_flag, set_max_thoughts_width, set_memory_model, set_multiline_mode,
    ...
    set_voice_stt_language, set_web_search_source, set_x_search_enabled,
};
...
pub(crate) fn dispatch(action: Action, app: &mut AppView) -> Vec<Effect> {
    ...
    let effects = match action {
        ...
        Action::SetWebSearchSource { key, choice } => set_web_search_source(app, key, choice),
        Action::SetXSearchEnabled(enabled) => set_x_search_enabled(app, enabled),
        Action::SetLocalFeatureFlag { key, enabled } => set_local_feature_flag(app, key, enabled),
        ...
        Action::SetPerplexityApiKey { key } => set_perplexity_api_key(app, key),
        Action::ClearPerplexityApiKey => clear_perplexity_api_key(app),
        ...
        Action::SetMemoryModel(v) => set_memory_model(app, v),
        Action::ClearMemoryModel => clear_memory_model(app),
        ...
        Action::SetAntigravitySubagents(v) => set_antigravity_subagents(app, v),
        Action::SetAntigravitySkipPermissions(v) => set_antigravity_skip_permissions(app, v),
        ...
        Action::LogoutCodex => dispatch_logout_codex(app),
        ...
        Action::JumpPickerSelect(entry_id) => dispatch_jump_picker_select(app, entry_id),
