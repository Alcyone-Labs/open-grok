...
/// Path to `$OPENGROK_HOME/pager.toml`.
pub fn pager_toml_path() -> PathBuf {
    grok_home().join("pager.toml")
}

/// User-facing label for the user grok directory (``~/.opengrok`` or ``$OPENGROK_HOME``).
///
/// Derived from resolved [`grok_home()`] vs `xai_grok_config::default_grok_home()`,
/// not from whether `OPENGROK_HOME` is set in the environment.
...
fn display_grok_home_prefix_for(home: &Path) -> String {
    if home == xai_grok_config::default_grok_home() {
        ...
        "$OPENGROK_HOME".to_string()
    }
}

/// User-facing path under [`grok_home()`], e.g. ``~/.opengrok/config.toml``.
...
mod tests {
    ...
    fn display_grok_home_prefix_default_install() {
        if std::env::var("OPENGROK_HOME").is_ok() {
            return;
        }
        assert_eq!(display_grok_home_prefix(), "~/.opengrok");
    }
    ...
    fn display_user_grok_path_joins_relative() {
        ...
        assert!(path.contains(".opengrok") || path.contains("$OPENGROK_HOME"));
    }
    ...
    fn abbreviate_path_uses_home_when_under_default_grok() {
        if let Ok(home) = std::env::var("HOME") {
            ...
            let full = format!("{home}/.opengrok/memory/MEMORY.md");
