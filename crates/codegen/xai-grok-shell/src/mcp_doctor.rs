//! `open-grok mcp doctor` -- runtime health check for MCP servers.
...
fn discover_servers(cwd: &Path) -> (Vec<ConfigSourceStatus>, Vec<DiscoveredServer>) {
    ...
    if user_config.is_file() {
        sources.push(ConfigSourceStatus {
            path: "~/.opengrok/config.toml".to_string(),
...
async fn try_discover_managed_servers() -> (ConfigSourceStatus, Vec<DiscoveredServer>) {
    ...
    let token = match auth_manager.get_valid_token().await {
        Ok(key) => key,
        Err(_) => return managed_skipped("auth expired — run `open-grok login`"),
    };
...
pub async fn run_doctor(cwd: &Path, name_filter: Option<&str>) -> DoctorReport {
    ...
    // Folder-trust gate: `open-grok mcp doctor` actually STARTS each server
    ...
    //
    // `remote = None` is intentional: standalone `open-grok mcp doctor` has no loaded
...
pub fn print_report(report: &DoctorReport) {
    ...
    if report.servers.is_empty() {
        ...
        println!("  Run `open-grok mcp add --help` to get started.");
        println!();
        return;
    }
    ...
    println!(
        ...
        if report.failing_count > 0 {
            " Run `open-grok mcp doctor --json` for full diagnostics."
        } else {
            ""
        }
    );
