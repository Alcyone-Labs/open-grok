...
impl SessionActor {
    ...
    /// or removed it from `~/.opengrok/config.toml` while we were
    /// sleeping.
    ///
    ...
    /// which parses `~/.opengrok/config.toml` + the project
    /// `.opengrok/config.toml`) on every call. The auto-restart task
    /// calls this at most:
