        ...
        tokio::select! {
                ...
                maybe_cmd = cmd_rx.recv() => {
                    ...
                    match cmd {
                        ...
                        SessionCommand::SetSessionModel { model_id, sampling_config, use_concise, apply_prompt_override, skip_prompt_rewrite, auto_compact_threshold_percent, agent_rebuild, resolved_tool_policy_override, responds_to } => {
                            let updated_model_id = session.handle_set_session_model(model_id, sampling_config, use_concise, apply_prompt_override, skip_prompt_rewrite, auto_compact_threshold_percent, agent_rebuild, resolved_tool_policy_override).await;
                            let _ = responds_to.send(updated_model_id);
                            SessionActor::maybe_start_running_task(session.clone(), completion_tx.clone()).await;
                            SessionActor::maybe_drain_notifications(session.clone(), completion_tx.clone()).await;
                        }
                        SessionCommand::ReloadWebSearchToolset { state, responds_to } => {
                            session.handle_reload_web_search_toolset(state, responds_to).await;
                            SessionActor::maybe_start_running_task(session.clone(), completion_tx.clone()).await;
                            SessionActor::maybe_drain_notifications(session.clone(), completion_tx.clone()).await;
                        }
                        ...
                        SessionCommand::CompactSession { user_context, respond_to } => {
                            session.start_manual_compaction(user_context, respond_to, completion_tx.clone()).await;
                        }
                        ...
                        SessionCommand::SetSwarmMode { enabled, trigger } => {
                            if enabled {
                                session.enter_swarm_mode(trigger).await;
                            } else {
                                let exited = {
                                    let mut state = session.state.lock().await;
                                    if trigger == crate::session::swarm_mode::SwarmModeTrigger::Task {
                                        state.swarm_mode.exit_if_trigger(trigger)
                                    } else {
                                        let was_enabled = state.swarm_mode.enabled();
                                        state.swarm_mode.exit();
                                        was_enabled
                                    }
                                };
                                if exited {
                                    session.send_xai_notification(crate::extensions::notification::SessionUpdate::SwarmModeChanged { enabled: false, trigger: None }).await;
                                }
                            }
                        }
                        ...
                        SessionCommand::RepairHistory { dry_run, respond_to } => {
                            session.start_history_repair(dry_run, respond_to, completion_tx.clone()).await;
                        }
                        ...
                        SessionCommand::SnapshotToolDefinitions { respond_to } => {
                            ...
                            let sampling_config = session.chat_state_handle.get_sampling_config().await;
                            let provider = sampling_config.as_ref().map(|config| config.provider).unwrap_or_default();
                            let specs = session.forked_tool_override.as_deref().map(|tools| session.provider_filtered_tool_specs(tools, provider)).unwrap_or_else(|| session.turn_base_tool_specs(&defs, provider));
                            let resolved_policy = sampling_config.as_ref().and_then(|config| session.snapshot_resolved_tool_policy(config));
                            let _ = respond_to.send(crate::session::tool_surface::ToolSurfaceSnapshot { function_tools: specs, resolved_policy });
                        }
