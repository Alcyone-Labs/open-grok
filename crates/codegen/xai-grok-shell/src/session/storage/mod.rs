...
//   - tests / pager fork replay:       `load_updates_for_replay_at` (typed reference)
//
// Open Grok keeps `load_updates_for_replay_at` public for the pager's inherited
// fork replay path until that caller switches to `stream_replay_updates_at`.
...
/// owned `Vec`s. Prefer [`stream_replay_updates_at`] for production fork/resume
/// paths so peak memory stays near the on-disk transcript size.
...
///
/// Open Grok routes through [`prepare_replay_lines_with_transport_ids`] so
/// Code Mode transport wrappers stay hidden on resume.
#[doc(hidden)]
pub fn prepare_replay_lines<'a>(contents: &'a str, cursor: Option<&str>) -> PreparedReplay<'a> {
    prepare_replay_lines_with_transport_ids(contents, cursor, &std::collections::HashSet::new())
}
