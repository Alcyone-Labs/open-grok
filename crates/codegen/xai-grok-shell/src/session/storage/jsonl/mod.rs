...
impl JsonlStorageAdapter {
    ...
    fn load_workflow_runs_sync(
        ...
        let mut entries: Vec<_> = std::fs::read_dir(&workflows_dir)?
            ...
            .collect();
        // Sort *before* truncating: `read_dir` yields entries in an arbitrary
        // order, so capping first would keep a non-deterministic subset and let
        // a later-rejected entry (symlink/invalid manifest) consume a cap slot,
        // silently dropping a valid run below the intended `MAX` ceiling.
