...
fn mid_turn_user_injection_must_not_duplicate_tool_results_for_one_tool_use_id() {
    let thread = std::thread::Builder::new()
        .name("chat-history-integrity".to_string())
        ...
        .spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                ...
                .block_on(run_mid_turn_user_injection_history_test());
        })
        .expect("spawn chat-history test thread");
    thread.join().expect("chat-history test thread");
}

async fn run_mid_turn_user_injection_history_test() {
