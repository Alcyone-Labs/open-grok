...
fn roles_parse_from_toml() {
    let toml_str = r#"
            ...
            prompt_file = ".opengrok/prompts/impl.md"
        "#;
    ...
    assert_eq!(implementer.prompt_file.as_deref(), Some(".opengrok/prompts/impl.md"));
}
#[test]
...
fn validate_roles_accepts_valid_prompt_file() {
    let toml_str = r#"
            ...
            prompt_file = ".opengrok/prompts/ok.md"
        "#;
    let cfg: SubagentsConfig = toml::from_str(toml_str).unwrap();
    assert!(cfg.validate_roles().is_empty());
}
...
fn discover_roles_loads_from_directory() {
    ...
    let roles_dir = tmp.path().join(".opengrok").join("roles");
...
fn personas_parse_from_toml() {
    ...
    assert_eq!(concise.instructions_file.as_deref(), Some(".opengrok/personas/concise.md"));
}
#[test]
...
fn discover_personas_loads_from_directory() {
    let tmp = tempfile::TempDir::new().unwrap();
    let dir = tmp.path().join(".opengrok").join("personas");
...
fn bundled_personas_and_roles_have_lowest_priority_in_resolve_order() {
    ...
    let bundled = home.join(".opengrok").join("bundled");
    std::fs::create_dir_all(workspace.join(".opengrok").join("roles")).unwrap();
    std::fs::create_dir_all(workspace.join(".opengrok").join("personas")).unwrap();
    std::fs::create_dir_all(home.join(".opengrok").join("roles")).unwrap();
    std::fs::create_dir_all(home.join(".opengrok").join("personas")).unwrap();
    ...
    std::fs::write(
        ...
        .unwrap();
    std::fs::write(
            home.join(".opengrok/roles/reviewer.toml"),
            r#"description = "User reviewer""#,
        )
        .unwrap();
    std::fs::write(
            home.join(".opengrok/personas/reviewer.toml"),
            r#"instructions = "User persona""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".opengrok/roles/reviewer.toml"),
            r#"description = "Project reviewer""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".opengrok/personas/reviewer.toml"),
    ...
    std::fs::remove_file(workspace.join(".opengrok/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(workspace.join(".opengrok/personas/reviewer.toml")).unwrap();
    let config = toml::from_str::<
        toml::Value,
    >(r#"
            [subagents]
    ...
    std::fs::remove_file(home.join(".opengrok/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(home.join(".opengrok/personas/reviewer.toml")).unwrap();
    let config = toml::from_str::<
        toml::Value,
    >(r#"
            [subagents]
...
fn render_io_summary_shows_bundled_for_bundled_personas() {
    let persona = SubagentPersona {
        ...
        source_path: Some("/tmp/home/.opengrok/bundled/personas/reviewer.toml".to_string()),
        ..Default::default()
    };
...
/// RCE guard: a project `.opengrok/config.toml` must never source
...
fn project_config_never_sources_feedback_user() {
    ...
    let grok = repo.path().join(".opengrok");
...
fn validate_hooks_path_rejects_outside_grok_home() {
    ...
    assert!(
        msg.contains("must be under ~/.opengrok/"),
        "should mention ~/.opengrok/ restriction, got: {msg}"
    );
}
#[test]
...
fn validate_hooks_path_accepts_grok_hooks_subdir() {
    ...
    assert!(result.is_ok(), "path under ~/.opengrok/ should be accepted");
}
#[test]
...
fn explicit_grok_root_is_the_only_user_source() {
    let tmp = tempfile::tempdir().unwrap();
    let ambient = tmp.path().join("ambient-home/.opengrok");
...
/// verdicts untouched). OPENGROK_HOME-isolated + `#[serial]` for folder-trust
/// store hygiene (empty store ⇒ deterministic untrusted;
/// `EnvGuard` restores OPENGROK_HOME even on panic). No user-global
/// `$OPENGROK_HOME/config.toml` is seeded: `grok_home()` is `OnceLock`-cached,
...
/// OPENGROK_HOME-isolated + `#[serial]` (`EnvGuard` restores it even on panic).
#[test]
#[serial_test::serial]
...
/// arm (store-only reconcile) could never lift. OPENGROK_HOME-isolated (empty
