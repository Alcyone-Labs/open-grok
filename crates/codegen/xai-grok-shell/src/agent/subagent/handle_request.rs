...
pub(crate) async fn run_shell_child(
    ...
    let provider_registry_guard = super::ProviderRegistryGuard::register(
    ...
    // Fork delta: an agent_swarm member resumed from a prior member adopts the
    // source's subagent_type/persona BEFORE definition resolution so the
    // resumed child rebuilds with the source identity.
    let pre_resolved_resume_source = if let Some(resume_id) = request
        .swarm
        .as_ref()
        ...
        .map(str::to_string)
    {
            ...
            .await
        {
            ...
            SubagentResumeLookup::Completed(info) => Some(
                durable_resume_source_for(&resume_id, &ctx.parent_session_id, &ctx.parent_cwd)
    ...
    if let Some(ref source) = pre_resolved_resume_source {
    ...
    let meta_model_route =
        resolved_model_route(&effective_sampling_config, &effective_model_id, &ctx);
    ...
    if let (Some(bucket_url), Some(upload_method)) =
        (&child_gcs_bucket_url, &child_gcs_upload_method)
    {
    ...
    let parent_surface_snapshot = (verbatim_mirror_fork && !request.owner.is_workflow())
    ...
    // Fork delta: agent_swarm children stream provider/rate-limit status to
    // the swarm scheduler once promoted to active.
    ...
    let _ = child_handle.cmd_tx.send(SessionCommand::Prompt {
        ...
        artifact_upload_ctx: ctx
            ...
            .as_ref()
            .and_then(|_| {
                child_gcs_upload_method.as_ref().map(|method| {
    ...
    let SubagentWaitResult {
        ...
        final_text_override,
    } = await_subagent_turn_with_summary_floor(&child_handle, prompt_rx, cancel_token.clone())
        .await;
    ...
    if let Some(ref wt_path) = worktree_path {
        if snapshot_dispose_enabled {
            let ref_name = format!("refs/open-grok/subagents/{}", request.id);
            let source_repo = resolve_subagent_source_repo(&ctx);
