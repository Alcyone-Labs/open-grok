...
pub(crate) struct SubagentSpawnContext {
    ...
    /// Resolved backend config for web_search.
    pub web_search_config: crate::agent::config::PreparedWebSearchConfig,
    ...
    /// Fork: registry handle for provider-scoped runtime-change cancellation.
    pub provider_registry: Option<SubagentProviderRegistry>,
...
/// Fork: live registry of in-flight children keyed by subagent id, tagged
/// with the child's resolved sampler provider. Used to fail closed on
/// Kimi/Fireworks runtime credential changes: children capture credentials in
/// their sampler at spawn, so provider-bound children (and children still
/// initializing, whose provider is not yet safely resolved) must stop before
/// the live credential mutates. Entries insert at `run_shell_child` entry and
/// drop with the run.
pub(crate) type SubagentProviderRegistry = std::rc::Rc<
    std::cell::RefCell<
        std::collections::HashMap<
            String,
            (
                Option<xai_grok_sampling_types::ModelProvider>,
                CancellationToken,
            ),
        >,
    >,
>;
pub(crate) struct ProviderRegistryGuard {
    registry: Option<SubagentProviderRegistry>,
    id: String,
}
impl ProviderRegistryGuard {
    pub(crate) fn register(
        ...
        if let Some(ref reg) = registry {
            reg.borrow_mut()
                .insert(id.to_owned(), (None, cancel_token.clone()));
        }
        Self {
            registry,
            id: id.to_owned(),
    ...
    pub(crate) fn set_provider(&self, provider: xai_grok_sampling_types::ModelProvider) {
        if let Some(ref reg) = self.registry
            && let Some(entry) = reg.borrow_mut().get_mut(&self.id)
        {
...
/// Cancel children bound to `provider`, plus every child whose provider has
/// not resolved yet. Returns the number of cancelled children.
pub(crate) fn cancel_for_provider_runtime_change(
    registry: &SubagentProviderRegistry,
    provider: xai_grok_sampling_types::ModelProvider,
) -> usize {
    let mut cancelled = 0;
    for (entry_provider, token) in registry.borrow().values() {
        if entry_provider.is_none() || *entry_provider == Some(provider) {
            token.cancel();
            cancelled += 1;
        }
    }
