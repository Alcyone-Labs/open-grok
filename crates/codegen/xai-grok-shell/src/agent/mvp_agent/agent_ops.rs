...
impl MvpAgent {
    ...
    pub(super) fn on_remote_settings_changed(&self) {
        ...
        if let Some(identity) = self
            ...
            .map(|a| a.user_id)
        {
            self.tier_allowed.set(super::settings_allow_access(
                self.cfg.borrow().remote_settings.as_ref(),
            ));
    ...
    fn reapply_official_marketplace(&self) {
        if self
            .cfg
            .borrow()
            .resolve_official_marketplace_auto_register()
            .value
        {
    ...
    async fn fetch_settings(
        ...
        match tokio::task::spawn_blocking(move || {
            crate::remote::fetch_settings_blocking(&base_url, &auth, alpha.as_deref())
        })
        .await
        {
            Ok(outcome) => outcome,
    ...
    pub(super) fn spawn_settings_reapply(&self) {
        ...
        let auth_manager = self.auth_manager.clone();
        let _spawned = self.spawn_coalesced_settings_task(
    ...
    pub fn with_models(
        ...
        let instance = Self {
            ...
            subagent_provider_registry: Default::default(),
    ...
    pub(super) fn insert_session_config_meta(
        ...
        {
            meta.insert(
                crate::session::SCHEDULER_BACKGROUND_LOOPS_META_KEY.to_string(),
