...
fn persisted_tool_policy_is_not_reused_for_unavailable_model_fallback() {
    ...
    assert!(
        acp_agent::LoadModelSelectionOrigin::PersistedIdentity.restores_persisted_tool_policy(
            None,
            xai_policy,
            xai_grok_sampling_types::ModelProvider::Xai,
            &xai_grok_sampling_types::ApiBackend::Responses,
    ...
    assert!(
        !acp_agent::LoadModelSelectionOrigin::UnavailableFallback.restores_persisted_tool_policy(
            None,
            xai_policy,
...
fn replay_recovers_xai_function_envelope_exec_and_wait_transport_ids() {
    ...
    let conversation =
        vec![xai_grok_sampling_types::ConversationItem::assistant_tool_calls(vec![exec, wait])];
...
async fn ensure_plugin_registry_lazily_populates_snapshot() {
    ...
    let _env = EnvGuard::set("OPENGROK_HOME", grok_home.path());
...
fn write_project_subagent_definitions(cwd: &std::path::Path) {
    let roles = cwd.join(".opengrok/roles");
    let personas = cwd.join(".opengrok/personas");
...
/// End-to-end gate wiring: project `.opengrok/roles` / `personas` alone must drive
