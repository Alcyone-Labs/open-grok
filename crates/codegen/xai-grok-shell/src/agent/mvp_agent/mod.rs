...
pub struct MvpAgent {
    ...
    /// **non-recursive** watch on `<cwd>/` and `<cwd>/.opengrok/`).
    ///
    ...
    /// Fork: provider-tagged live-children registry for runtime credential
    /// changes (see `subagent::cancel_for_provider_runtime_change`).
    subagent_provider_registry: crate::agent::subagent::SubagentProviderRegistry,
...
impl MvpAgent {
    ...
    /// blocks; an absent field fails open). When settings have not arrived yet
    ...
    pub(super) async fn enforce_grok_code_access(&self, auth: &crate::auth::GrokAuth) {
        ...
        {
            // Settings still background-fetching: provisional open.
            self.tier_allowed.set(true);
            return;
        }
    ...
    ///    against `~/.opengrok/bundled/` and the manifest.
