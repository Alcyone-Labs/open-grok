...
pub(super) fn deepseek_models_apply_payload(
    refreshed: Result<bool, String>,
    models: acp::SessionModelState,
) -> serde_json::Value {
    match refreshed {
        Ok(refreshed) => serde_json::json!({ "refreshed": refreshed, "models": models }),
        Err(warning) => {
            tracing::warn!(%warning, "DeepSeek model query failed; returning embedded models");
            serde_json::json!({ "refreshed": false, "warning": warning, "models": models })
        }
    }
}
    ...
    async fn authenticate(
        ...
        match arguments.method_id.0.as_ref() {
            ...
            auth_method::GROK_COM_METHOD_ID | auth_method::OIDC_METHOD_ID => {
                ...
                self.spawn_post_auth_settings(auth.clone());
                Ok(self.auth_response_with_meta())
            }
            _ => {
                Err(
        ...
        let result = match method.as_ref() {
            ...
            "open-grok/deepseek/models/apply" => {
                ...
                if cancelled_subagents > 0 {
                    tracing::warn!(
                        cancelled_subagents,
                        "cancelled subagents before DeepSeek runtime credential change"
                    );
                }
                let refreshed = self
                    .models_manager
                    .apply_deepseek_credential_change()
                    .await
                    .map_err(|error| error.to_string());
                let available = self.models_manager.available();
                let models = acp::SessionModelState::new(
            ...
            "x.ai/cloud/terminate" => {
                crate::extensions::auth_gate::require_xai_auth(
                    ...
                    "Run `open-grok login` to authenticate.",
                )?;
