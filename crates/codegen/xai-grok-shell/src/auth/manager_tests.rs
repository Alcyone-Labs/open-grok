...
async fn auth_returns_expired_api_key_consistently_with_current() {
    ...
    // funnel the user back through `open-grok login`.
    let err = mgr.auth().await.unwrap_err();
    assert!(
...
async fn refresh_chain_demotes_when_attributed_tried_rt_differs_from_disk() {
    ...
    assert!(
        ...
        "demotion must not record a sticky verdict that locks out every \
         sibling process until the user re-runs `open-grok login`",
    );
}
...
async fn permanent_failure_reads_absent_after_clear_so_auth_reports_not_logged_in() {
    ...
    // User runs `open-grok logout` which calls clear().
    mgr.clear().unwrap();

    // The diagnostic the user now sees on the next request should be
    // "Not logged in. Run `open-grok login`.", not the stale invalid_grant.
    let err = mgr.auth().await.unwrap_err();
    assert!(
...
#[tokio::test]
#[serial_test::serial] // reaches `resolve_static_api_key`, which reads key env vars
...
/// `preferred_method=api_key`: leftover session must not beat a static API key.
#[tokio::test]
#[serial_test::serial]
