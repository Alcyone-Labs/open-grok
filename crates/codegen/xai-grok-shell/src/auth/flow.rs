...
async fn run_auth_flow_inner(
    ...
    // `open-grok login --devbox` uses run_devbox_login and is not gated here.
    ...
    anyhow::bail!(
        "No OAuth2 configuration available. Run `open-grok login` to authenticate, or contact your administrator if you use enterprise SSO."
    )
}
...
/// Unified `open-grok login` handler for CLI entry points (tui, pager).
///
...
pub async fn run_cli_login(
    ...
    // supports the device flow. Without this guard, `open-grok login` on an
...
/// `open-grok logout` CLI handler. Calls [`perform_logout`] and formats
...
mod tests {
    ...
    fn expired_refreshable_session_gate() {
        ...
        // An expired first-party external credential with a refresh token is
        // likewise recoverable: 401 recovery re-runs the provider binary (the
        // refresh token is a recoverability marker, not a grant input).
        mgr.hot_swap(GrokAuth {
        ...
        assert_eq!(
            expired_refreshable_session(&mgr).map(|auth| auth.key),
    ...
    async fn enterprise_oidc_never_uses_device_flow() {
        // oidc=Some, oauth2=None: `open-grok login` must use loopback, not device —
    ...
    fn external_cred_compatibility_follows_issuer() {
        ...
        // A first-party external credential is reused by interactive login
        // like an OIDC session instead of re-running the provider.
        assert!(is_cached_credential_compatible(
            &GrokAuth {
                auth_mode: AuthMode::External,
                ..oidc_auth(XAI_OAUTH2_ISSUER)
        ...
        // Bare-token providers carry no issuer and remain incompatible.
        assert!(!is_cached_credential_compatible(
            &GrokAuth {
                auth_mode: AuthMode::External,
                oidc_issuer: None,
    ...
    /// CLI `open-grok login` passes `on_stderr=None`; stderr must be inherited so
