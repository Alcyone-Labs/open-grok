...
//!
//! The tests exercise:
//! - **Smoke** (`open-grok --version`): binary loads without crashing
//! - **ACP stdio** (`open-grok agent stdio`): full protocol lifecycle via ClientSideConnection
//!
...
//
// These test the agent as a server: spawn `open-grok agent stdio`, speak the full
...
/// `open-grok agent stdio` must initiate shutdown and exit when its client closes
...
async fn test_stdio_agent_exits_on_stdin_eof() {
    ...
    let mut process = TestProcess::spawn(
        ...
        TestProcessConfig::new()
            .label("open-grok agent stdio (eof)")
            .stdin(TestStdin::Piped)
            .stdout(TestOutput::Piped),
    )
    .expect("spawn open-grok agent stdio");
    ...
    let status = process
        ...
        .unwrap_or_else(|| {
            panic!(
                "open-grok agent stdio did not exit after stdin EOF\n{}",
...
/// Isolated headless run with a custom `~/.opengrok/`. Clean env (no leaked
